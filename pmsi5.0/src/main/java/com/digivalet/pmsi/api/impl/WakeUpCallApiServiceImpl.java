...
public class WakeUpCallApiServiceImpl extends WakeUpCallApiService
{
   static DVLogger dvLogger = DVLogger.getInstance();

   @Override
   public Response setResetWakeUpCall(String accessToken,
            @NotNull String hotelCode, @NotNull String roomNumber,
            @NotNull String wakeUpTime, @NotNull String wakeUpDate,
            @NotNull Boolean operation, SecurityContext securityContext)
            throws NotFoundException
   {
      JSONObject tokenVerify = null;
      Thread.currentThread().setName("WAKEUP-CALL-API");
      dvLogger.info(" Inside set wake up call API");
      try
      {
      ...
      if (valid.getCode() == DVResult.SUCCESS)
      {
         try
         {
            DVPmsMain.getInstance().getDvPmsController().postWakeupCall(
                     roomNumber, wakeUpDate, wakeUpTime, operation);
